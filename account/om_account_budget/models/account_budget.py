    ...
    def _compute_practical_amount(self):
        for line in self:
            ...
            if line.analytic_account_id.id:
                ...
                if not analytic_line_obj.env.su:
                    rule_domain = self.env['ir.rule']._compute_domain(analytic_line_obj._name, 'read')
                    domain = list(Domain.AND([domain, rule_domain]))
                ...
                if not aml_obj.env.su:
                    rule_domain = self.env['ir.rule']._compute_domain(aml_obj._name, 'read')
                    domain = list(Domain.AND([domain, rule_domain]))
