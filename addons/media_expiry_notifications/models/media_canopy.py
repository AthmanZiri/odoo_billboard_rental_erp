...
class MediaCanopy(models.Model):
    _name = 'media.canopy'
    _inherit = ['media.canopy', 'media.expiry.mixin']

    @api.model
    def _cron_notify_expiring_canopies(self):
        days = self._get_notification_days()
        target_date = fields.Date.today() + relativedelta(days=days)
        today = fields.Date.today()

        canopies = self.search([('status', 'in', ['active', 'damaged'])])
        for canopy in canopies:
            active_line = canopy.lease_line_ids.filtered(
                lambda l: l.state in ['sale', 'done'] 
                and l.start_date and l.end_date 
                and l.start_date <= today <= l.end_date
            ).sorted(key=lambda l: l.end_date, reverse=True)

            active_history = canopy.site_id.artwork_history_ids.filtered(
            ...
            if end_date == target_date:
                ...
                self._trigger_expiry_notification(
                    asset_type='Canopy',
                    asset_name=canopy.display_name,
                    end_date=end_date,
                    user_id=user_id,
                    asset_record=canopy
                )
