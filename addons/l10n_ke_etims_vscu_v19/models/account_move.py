...
class AccountMove(models.Model):
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        self.filtered(lambda m: m.l10n_ke_oscu_invoice_number).show_reset_to_draft_button = True
            ...
            if move.move_type in ('out_refund', 'in_refund'):
                ...
                if not move.reversed_entry_id:
                    messages['no_reversed_entry_warning'] = {
                        'message': _("The original invoice being reversed is required for credit notes. "),
                        'blocking': True,
                    }
    ...
    def _l10n_ke_oscu_json_from_move(self):
        ...
        tax_codes = {item['code']: item['tax_rate'] for item in self.env['l10n_ke_etims_vscu_v19.code'].search([('code_type', '=', '04')])}
        ...
        if self.is_purchase_document(include_receipts=True):
            ...
            if self.partner_id.phone:
                receipt_part.update({
                    'custMblNo': (self.partner_id.phone or '')[:20]       # Mobile number, not required
                })
    ...
    def _l10n_ke_oscu_get_json_from_lines(self, tax_details):
        ...
        for index, line in enumerate(self.invoice_line_ids.filtered(lambda l: l.display_type not in ('line_section', 'line_note'))):
            ...
            tax, line_tax_details = next(
                ...
                if tax.vscu_l10n_ke_tax_type_id  # We only want to report VAT taxes
            )
            ...
            line_values = {
                ...
                'pkgUnitCd': product.vscu_l10n_ke_packaging_unit_id.code,                # Packaging code, describes the type of package used
                'pkg':       product_uom_qty / product.l10n_ke_packaging_quantity,  # Number of packages used
                'qtyUnitCd': line.product_uom_id.vscu_l10n_ke_quantity_unit_id.code,     # The UOMs as defined by the KRA, defined seperately from the UOMs on the line
                'qty':       line.quantity,
                'prc':       price_unit,
                'splyAmt':   price_subtotal_before_discount,
                'dcRt':      line.discount,
                'dcAmt':     discount_amount,
                'taxTyCd':   tax.vscu_l10n_ke_tax_type_id.code,
    ...
    def _post(self, soft=True):
        ...
        for move in self.filtered(lambda move: move.country_code == 'KE' and move.reversed_entry_id):
            ...
            for product, quantity in reverse_quantities.items():
                ...
                product_name = product.name or f"Product ID {product.id}"
                if product not in original_quantities:
                    exceeding_quantities.append(_("'%s' is not present on the original invoice.", product_name))
                    ...
                    exceeding_quantities.append(
                        _(
                            ...
                            excess=excess,
                            uom_name=product.uom_id.name or "units"
                        )
                    )
            ...
            if exceeding_quantities:
                if len(reversals) > 1:
                    raise UserError(_(
                        ...
                        other_credit_notes=', '.join(filter(None, (reversals - move).mapped("name"))),
    ...
    def _l10n_ke_oscu_send_customer_invoice(self):
        self.ensure_one()
        if self.move_type in ('out_refund', 'in_refund'):
            if not self.l10n_ke_reason_code_id:
                raise UserError(_("A KRA reason code is required for credit notes."))
            if not self.reversed_entry_id:
                raise UserError(_("The original invoice being reversed is required for credit notes."))
    ...
    def _get_edi_decoder(self, file_data, new=False):
        # EXTENDS 'account'
        if file_data.get('type') == 'binary' and file_data.get('content') and self._is_vendor_bill_json(file_data['content']):
    ...
    def _get_name_invoice_report(self):
        ...
        if self.l10n_ke_oscu_invoice_number:
            return 'l10n_ke_etims_vscu_v19.report_invoice_document'
