...
from odoo.addons.l10n_ke_etims_vscu_v19.models.account_move import format_etims_datetime
...
class ProductTemplate(models.Model):
    ...
    vscu_l10n_ke_packaging_unit_id = fields.Many2one(
        comodel_name='l10n_ke_etims_vscu_v19.code',
        string="Packaging Unit",
        compute='_compute_vscu_l10n_ke_packaging_unit_id',
        inverse='_set_vscu_l10n_ke_packaging_unit_id',
        domain=[('code_type', '=', '17')],
    ...
    @api.depends('product_variant_ids.vscu_l10n_ke_packaging_unit_id')
    def _compute_vscu_l10n_ke_packaging_unit_id(self):
        self._compute_template_field_from_variant_field('vscu_l10n_ke_packaging_unit_id')

    def _set_vscu_l10n_ke_packaging_unit_id(self):
        self._set_product_variant_field('vscu_l10n_ke_packaging_unit_id')
    ...
    def _get_related_fields_variant_template(self):
        ...
        return [
            *super()._get_related_fields_variant_template(),
            'vscu_l10n_ke_packaging_unit_id',
...
class ProductProduct(models.Model):
    ...
    @api.depends('type')
    def _compute_vscu_l10n_ke_packaging_unit_id(self):
        service_packaging = self.env.ref('l10n_ke_etims_vscu_v19.packaging_type_ou', raise_if_not_found=False)
        for product in self.filtered(lambda p: not p.vscu_l10n_ke_packaging_unit_id):
            product.vscu_l10n_ke_packaging_unit_id = service_packaging if product.type == 'service' else None
    ...
    @api.depends(
        ...
        'l10n_ke_origin_country_id',
        'l10n_ke_product_type_code',
        'taxes_id.vscu_l10n_ke_tax_type_id',
    )
    ...
    def _l10n_ke_get_validation_messages(self, for_invoice=False):
        ...
        products_missing_fields = self.filtered(
            lambda p: (
                not p.unspsc_code_id or not p.vscu_l10n_ke_packaging_unit_id
        ...
        products_incorrect_taxes = self.filtered(
            lambda p: len(p.taxes_id.filtered(lambda t: t.vscu_l10n_ke_tax_type_id and t.company_id in self.env.company.parent_ids)) > 1
        )
        ...
        products_mismatched_taxes = self.filtered(
            lambda p: p.taxes_id.filtered(
                lambda t: (
                    t.vscu_l10n_ke_tax_type_id
                    and t.company_id in self.env.company.parent_ids
                    and p.unspsc_code_id.vscu_l10n_ke_tax_type_id
                    and t.vscu_l10n_ke_tax_type_id != p.unspsc_code_id.vscu_l10n_ke_tax_type_id
                )
            )
        )
        if products_mismatched_taxes:
            messages['product_mismatched_taxes'] = {
                'message': _(
                    ...
                    product_tax_code=products_mismatched_taxes[0].taxes_id.filtered(lambda t: t.company_id in self.env.company.parent_ids).vscu_l10n_ke_tax_type_id.code,
                    unspsc_tax_code=products_mismatched_taxes[0].unspsc_code_id.vscu_l10n_ke_tax_type_id.code,
                ),
                'action_text': _("View Product(s)"),
                'action': products_incorrect_taxes._get_records_action(name=_("View Product(s)"), context={}),
            }
    ...
    def _l10n_ke_get_tax_type(self):
        ...
        self.ensure_one()
        return self.taxes_id.filtered(lambda t: t.company_id in self.env.company.parent_ids).vscu_l10n_ke_tax_type_id
    ...
    def _l10n_ke_action_open_products(self, title=None):
        ...
        res = {
            ...
            'view_mode': 'tree',
            'views': [(self.env.ref('l10n_ke_etims_vscu_v19.l10n_ke_kra_product_tree').id, 'tree'), (False, 'form')],
    ...
    def _calculate_l10n_ke_item_code(self):
        ...
        code_fields = [
            ...
            self.vscu_l10n_ke_packaging_unit_id.code,
            self.uom_id.vscu_l10n_ke_quantity_unit_id.code,
        ]
        _logger.info("code_fields: %s", code_fields)
    ...
    def _l10n_ke_oscu_save_item_content(self):
        ...
        content = {
            ...
            'pkgUnitCd':   self.vscu_l10n_ke_packaging_unit_id.code,                   # Packaging unit code
            'qtyUnitCd':   self.uom_id.vscu_l10n_ke_quantity_unit_id.code,             # Quantity unit code
    ...
    def _l10n_ke_oscu_save_item(self):
        ...
        error, data, _date = self.env.company._l10n_ke_call_etims('items/saveItems', content)
        if not error:
...
class ProductCode(models.Model):
    ...
    vscu_l10n_ke_tax_type_id = fields.Many2one('l10n_ke_etims_vscu_v19.code')
    ...
    def _cron_l10n_ke_oscu_get_codes_from_device(self):
        ...
        tax_codes = {
            ...
            for tax_code in self.env['l10n_ke_etims_vscu_v19.code'].search_read([('code_type', '=', '04')], ['code'])
        }
        ...
        for code in existing_codes:
            if (new_tax_code := not code.vscu_l10n_ke_tax_type_id and cls_list[code.code]['taxTyCd']):
                code.write({'vscu_l10n_ke_tax_type_id': tax_codes.get(new_tax_code)})
        ...
        new_codes = self.env['product.unspsc.code'].create([
            {
                ...
                'code': code,
                'applies_to': 'product',
                'vscu_l10n_ke_tax_type_id': tax_codes.get(code_dict['taxTyCd']),
                'active': True,
            }
